...
E-mail: $if(email)$\texttt{$email$}$else$TBD$endif$ & Web: $if(web)$\href{http://$web$}{\tt $webname$ }$else$TBD$endif$\\
Horario de atención: $if(officehours)$$officehours$$else$TBD$endif$  &  Horario de Clase: $if(classhours)$$classhours$$else$TBD$endif$\\
Oficina: $if(office)$$office$$else$TBD$endif$  & Sala de Clases: $if(classroom)$$classroom$$else$TBD$endif$\\
	&  \\
	\hline
	\end{tabular*}
